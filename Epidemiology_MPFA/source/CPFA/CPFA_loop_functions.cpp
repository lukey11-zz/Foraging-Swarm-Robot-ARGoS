...
void CPFA_loop_functions::Reset() {
  ...
  score = 0.0;
	...
	FoodStatus.clear(); //05/07/2021
    FoodTimer.clear();//qilu 05/10/2021
    
    
    TargetRayList.clear();
    
    SetFoodDistribution();
    InitInfectedFood(4); //qilu 05/07/2021
...
void CPFA_loop_functions::PreStep() {
    ...
    if(GetSpace().GetSimulationClock() > ResourceDensityDelay)
    {
        ...
        {   if(FoodStatus[i] == 0)
            ...
            else if(FoodStatus[i] == 1)
                color = argos::CColor::RED;
            else color = argos::CColor::ORANGE;
            
            FoodColoringList[i] = color;
        }
  	 }  
...
bool CPFA_loop_functions::IsExperimentFinished() {
	...
	if(FoodList.size() == 0 || GetSpace().GetSimulationClock() >= MaxSimTime) {
        ...
        if(score >= NumDistributedFood*0.80){
		isFinished = true;
	}
...
void CPFA_loop_functions::PostExperiment() {
    ...
    if (PrintFinalScore == 1) {
	 string type="";
        ...
        dataOutput << "Score: "<< Score() << ", Time: "<<(CollisionTime-16*Score())/(2*ticks_per_second)<< "s, "<< curr_time_in_minutes <<", Seed: "<<RandomSeed<<endl;
        dataOutput.close();
...
void CPFA_loop_functions::InitInfectedFood(int num)
{
    for(size_t i=0; i< num; i++){
        ...
        FoodStatus[random] = 1;
        FoodColoringList[random] = CColor::RED;
        FoodTimer[random] = getSimTimeInSeconds();//qilu 05/10/2021
        }
    }
...
void CPFA_loop_functions::RandomFoodDistribution() 
{
	...
	for(size_t i = 0; i < FoodItemCount; i++) {
        ...
        FoodStatus.push_back(0); //05/07/2021
        FoodTimer.push_back(0); //05/10/2021
	}
}
...
argos::Real CPFA_loop_functions::getSimTimeInSeconds() {
	int ticks_per_second = GetSimulator().GetPhysicsEngine("dyn2d").GetInverseSimulationClockTick();
