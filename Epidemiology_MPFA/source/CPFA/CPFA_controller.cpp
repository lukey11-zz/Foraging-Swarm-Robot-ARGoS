...
void CPFA_controller::Returning() {
 //LOG<<"Returning..."<<endl;
	...
	if(IsInTheNest() && ClosestNest->GetTravelFlag() ==0) {
...
void CPFA_controller::SetHoldingFood() {
	...
	if(IsHoldingFood() == false) {
		...
		// current position and update the food list if we have.
            ...
            std::vector<int> newFoodStatus;//qilu 05/07/2021
            std::vector<argos::Real> newFoodTimer;//qilu 05/10/2021
      ...
      if(MPFA_state != RETURNING){
		         for(i = 0; i < LoopFunctions->FoodList.size(); i++) {
                      ...
                      //Return this unfound-food position to the list
                            ...
                            newFoodStatus.push_back(LoopFunctions->FoodStatus[i]); //qilu 05/07/2021
                            newFoodTimer.push_back(LoopFunctions->FoodTimer[i]);//qilu 05/10/2021
                         }
                 }
      }
      if(j>0){
          for(; j < LoopFunctions->FoodList.size(); j++) {
              ...
              newFoodStatus.push_back(LoopFunctions->FoodStatus[j]);
              newFoodTimer.push_back(LoopFunctions->FoodTimer[j]);//qilu 05/10/2021
          }
      }
      ...
      if(IsHoldingFood()) {
         ...
         LoopFunctions->FoodStatus = newFoodStatus;
         LoopFunctions->FoodTimer = newFoodTimer; //qilu 05/10/2021
